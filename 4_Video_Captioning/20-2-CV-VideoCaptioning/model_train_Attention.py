...
'''
from AttentionClass import BahdanauAttention
...
class vedio2text:
    ...
    def build_model(self, feat, captions=None, cap_len=None, sampling=None, phase=0):
        ...
        attention = BahdanauAttention(n_hidden)
        fc1 = tf.keras.layers.Dense(n_hidden)
        fc2 = tf.keras.layers.Dense(self.vocab_num)
        gru = tf.keras.layers.GRU(n_hidden,
        ...
        for i in range(0, max_caption_len):
            ...
            if i == 0:
                with tf.variable_scope("LSTM2"):
                    ...
                    '''
                    h_src = tf.reshape(h_src, (-1, h_src.shape[2]))
                    ...
                    '''
                    context_vector, attention_weights = attention(h_src, state_gre)
