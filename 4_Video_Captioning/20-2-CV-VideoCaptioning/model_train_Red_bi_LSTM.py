...
n_hidden = 128# LSTM Hidden State Size
...
class vedio2text:
    ...
    def build_model(self, feat, captions=None, cap_len=None, sampling=None, phase=0):
        ...
        with tf.variable_scope('LSTM1'):
            ...
            # lstm_red_fw = tf.nn.rnn_cell.BasicLSTMCell(n_hidden, forget_bias=forget_bias_red, state_is_tuple=True)
            # lstm_red_bw = tf.nn.rnn_cell.BasicLSTMCell(n_hidden, forget_bias=forget_bias_red, state_is_tuple=True)
            # if phase == phases['train']:
            #     lstm_red_fw = tf.contrib.rnn.DropoutWrapper(lstm_red_fw, output_keep_prob=dropout_prob)
        ...
        with tf.variable_scope('LSTM2'):
            ...
            lstm_gre_fw = tf.nn.rnn_cell.BasicLSTMCell(n_hidden, forget_bias=forget_bias_gre, state_is_tuple=True)
            lstm_gre_bw = tf.nn.rnn_cell.BasicLSTMCell(n_hidden, forget_bias=forget_bias_gre, state_is_tuple=True)
            if phase == phases['train']:
                lstm_gre_fw = tf.contrib.rnn.DropoutWrapper(lstm_gre_fw, output_keep_prob=dropout_prob)
                lstm_gre_bw = tf.contrib.rnn.DropoutWrapper(lstm_gre_bw, output_keep_prob=dropout_prob)

        # 초기 LSTM zero state로 설정
        # state_red_fw = lstm_red_fw.zero_state(batch_size, dtype=tf.float32)
        ...
        state_gre_fw = lstm_gre_fw.zero_state(batch_size, dtype=tf.float32)
        ...
        for i in range(0, n_frames):
            with tf.variable_scope("LSTM1"):
                # output_red_fw, state_red_fw = lstm_red_fw(image_emb[i,:,:], state_red_fw)
                # output_red_bw, state_red_bw = lstm_red_bw(image_emb[i, :, :], state_red_bw)
                #
                # output_red = tf.concat([output_red_fw, output_red_bw],axis=1)
            ...
            with tf.variable_scope("LSTM2"):
                ...
                output_gre_fw, state_gre_fw = lstm_gre_fw(tf.concat([padding, output_red], axis=1), state_gre_fw)
                output_gre_bw, state_gre_bw = lstm_gre_bw(tf.concat([padding, output_red], axis=1), state_gre_bw)
                output_gre = tf.concat([output_gre_fw, output_gre_bw], axis=1)
        ...
        for i in range(0, max_caption_len):
            with tf.variable_scope("LSTM1"):
                ...
                # output_red_fw, state_red_fw = lstm_red_fw(padding_in, state_red_fw)
            ...
            if i == 0:
                with tf.variable_scope("LSTM2"):
                    ...
                    output_gre_fw, state_gre_fw = lstm_gre_fw(con, state_gre_fw)
                    output_gre_bw, state_gre_bw = lstm_gre_fw(con, state_gre_bw)
                    output_gre = tf.concat([output_gre_fw, output_gre_bw], axis=0)
            else:
                if phase == phases['train']:
                    if sampling[i] == True:
                ...
                with tf.variable_scope("LSTM2"):
                    ...
                    output_gre = tf.concat([output_gre_fw, output_gre_bw], axis=0)
                    #state_gre = tf.concat([state_gre_fw, state_gre_bw], axis=1)
